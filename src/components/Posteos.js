...
import contenidoPosts from './data.json'
...
export default function Posteos({aumentarL}) {
  return (
    <div className='container'>
      {contenidoPosts.map(item => <Post key={item.id} post={item} name={item.titulo} aumentarL={aumentarL}/>)}
    </div>
  )
}
